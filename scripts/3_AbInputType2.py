...
def usage():
    print("To get fingerprint of cdr pdb file.\n")
...
cdr_zip_file_path = sys.argv[1]
jobid = sys.argv[2]

if cdr_zip_file_path[-4:] == '.zip':
    os.system('unzip '+cdr_zip_file_path+' -d '+'../user_data/'+jobid+'/input_files/ab_structure_file_path/')

aaindexf = '../bin/usedAAindex.csv'
cdr_pdb_path = '../user_data/'+jobid+'/input_files/ab_structure_file_path/'
cdrFingerFopath = '../user_data/'+jobid+'/calculate_files/CDRFingerPrint.txt'
for i in os.listdir(cdr_pdb_path):
    main(aaindexf,cdr_pdb_path+i,cdrFingerFopath)    
